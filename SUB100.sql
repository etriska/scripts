create or replace PACKAGE BODY fmaxC_18 AS
-- Global variables
--Saved record
...
chrSaved_subledger_type		VARCHAR2(1);
numSaved_amount			NUMBER(14,2);
numSaved_hrs			NUMBER(14,2);
numAct_hrs			NUMBER(14,2);
numAct_cost			NUMBER(12,4);
numPercentage			NUMBER(5,2);
...
CURSOR curMb(chrMb_thru_date_cur VARCHAR2,intTenant_arg NUMBER) IS
	SELECT proposal,TO_CHAR(tranx_date,'FXYYYYMMDD'),funding_code,
		AMOUNT,NVL(SUBLEDGER_TYPE,'M'),NVL(POST_FLAG,'N'),OFFSETFUNDING_CODE,
		offsetsub_code,sort_code,tranx_no,tranx_type, source_tranx_no,source_tranx_item
		FROM ae_s_fnd_a
...
-- Per Meenal, effective February 2012, WO_JRNL is processed in variant fmaxC_88, not here.
		AND tranx_type IN('TW','CW','AJ','WX','CJ','TP','OC','PO_INV_OC','WO_JRNL','SS')--'MARKUP')
		AND proposal IN(
				SELECT proposal
					FROM ae_p_pro_e
					WHERE order_type IN('C','P','U','E')
					--AWI - AIM 6.0 - MULTITENANCY - BEGIN
					AND MULTITENANT_ID = intTenant_arg
					--AWI - AIM 6.0 - MULTITENANCY - END
					-- 04/29/2011
--					AND oc_code<>'601220'
				)
		...
		AND MULTITENANT_ID = intTenant_arg
		--AWI - AIM 6.0 - MULTITENANCY - END
--and trunc(bill_date)='29-aug-2014'
...
numMb_source_tranx_item		NUMBER(16);
chrHold_fund_code		VARCHAR2(50);
chrRetry_yn			VARCHAR2(1);
CURSOR curPlantsub(intTenant_arg NUMBER) Is
...
REGEXP_SUBSTR(item_desc,'([['||CHR(32)||'-'||chr(127)||']]*)*'),
		...
		FROM plantsub;
--		--AWI - AIM 6.0 - MULTITENANCY - BEGIN
--		WHERE MULTITENANT_ID = intTenant_arg;
--		--AWI - AIM 6.0 - MULTITENANCY - END
PROCEDURE proc_Sub100(chrBegin_date VARCHAR2,chrEnd_date VARCHAR2,intTenant_arg NUMBER) IS
BEGIN
...
--chrTr_manufacturer_name:=	'MANUFACTURER_NAMExxxxxxxxxxxxx';
--chrTr_model_num:=		'MODEL_NUMxxxxxxxxxxxxxxxxxxxxx';
--chrTr_serial_num:=		'SERIAL_NUMxxxxxxxxxxxxxxx';
--chrTr_um_tag_num:=		'TAG_NUMxxxxx';

	EXECUTE IMMEDIATE 'DELETE FROM plantsub';-- WHERE MULTITENANT_ID = '||TO_CHAR(intTenant_arg);
	EXECUTE IMMEDIATE 'DELETE FROM plant_zero';-- WHERE MULTITENANT_ID = '||TO_CHAR(intTenant_arg);
	EXECUTE IMMEDIATE 'DELETE FROM audit_control';-- WHERE MULTITENANT_ID = '||TO_CHAR(intTenant_arg);
	EXECUTE IMMEDIATE 'DELETE FROM sub_cust_est_errors';-- WHERE MULTITENANT_ID = '||TO_CHAR(intTenant_arg);
	EXECUTE IMMEDIATE 'DELETE FROM sub_vendorid_errors';-- WHERE MULTITENANT_ID = '||TO_CHAR(intTenant_arg);
	...
	EXECUTE IMMEDIATE 'DELETE FROM sub_grand_totals';-- WHERE MULTITENANT_ID = '||TO_CHAR(intTenant_arg);
-- reset totals
...
--set dates, since user didn't specify date values or they were invalid
	...
	OPEN curMb(chrTodays_date,intTenant_arg);--'2007/12/07'
...
--						chrMb_offsetfunding_code,intTenant_arg);
			ELSE
...
-- 9/26/2014 changes to Plantsub layout. Group by subledger_type
				OR chrMb_subledger_type<>chrSaved_subledger_type
...
-- add record to plantsub table
					...
					proc_WriteOutCurrentRecord(intTenant_arg);
				ELSE
-- sum of debit and credit for those charges is zero
					--5005 Write-out-zero-recs
					proc_WriteOutZeroRecs(intTenant_arg);
				END IF;
...
-- On 9/26/2014. Changes to Plantsub layout. Compute hours
				BEGIN
					SELECT act_hrs,act_cost INTO numAct_hrs,numAct_cost
						FROM ae_p_wka_d
						WHERE trans_no=TO_NUMBER(TRIM(chrMb_source_Tranx_no))
						AND item_no=numMb_source_Tranx_item;
				EXCEPTION WHEN NO_DATA_FOUND THEN
					numAct_hrs:=0;
				END;
				chrRetry_yn:='N';
--dbms_output.put_line('n '||chrMb_proposal||' '||chrMb_sort_code||' '||chrMb_funding_Code||' '||chrMb_tranx_date);
				BEGIN
					SELECT sub_percent INTO numPercentage
						FROM ae_p_phs_c
						WHERE proposal=chrMb_proposal
						AND sort_code=chrMb_sort_code
						AND funding_code=chrMb_funding_Code
						AND (NVL(TRUNC(start_Date),TO_DATE(chrMb_tranx_date,'YYYYMMDD'))<=TO_DATE(chrMb_tranx_date,'YYYYMMDD')
							AND NVL(expire_Date,TO_DATE(chrMb_tranx_date,'YYYYMMDD'))>=TO_DATE(chrMb_tranx_date,'YYYYMMDD'));
				EXCEPTION WHEN NO_DATA_FOUND THEN chrRetry_yn:='Y';
          WHEN TOO_MANY_ROWS then dbms_output.put_line('n '||chrMb_proposal||' '||chrMb_sort_code||' '||chrMb_funding_Code||' '||chrMb_tranx_date) ;
				END;
				IF(chrRetry_yn='Y') THEN -- search based on funding code
					chrRetry_yn:='N';
					BEGIN
						SELECT sub_percent INTO numPercentage
							FROM ae_p_Phs_c
					...
					END;
				END IF;
				IF(chrRetry_yn='Y') THEN -- search based on posting dates
					chrRetry_yn:='N';
					BEGIN
						SELECT sub_percent INTO numPercentage
							FROM ae_p_Phs_c
...
--							AND NVL(expire_Date,TO_DATE(chrMb_tranx_date,'YYYYMMDD'))>=TO_DATE(chrMb_tranx_date,'YYYYMMDD'));
					...
					END;
				END IF;
--if(chrMb_tranx_type<>'WO_JRNL') then
--dbms_output.put_line(chrMb_proposal||' '||chrMb_sort_code||' '||chrMb_funding_code||' '||chrMb_tranx_date||' tc '||chrMb_source_tranx_no);
--end if;
				IF(chrRetry_yn='Y') THEN
dbms_output.put_line('Error: No account or date matches for:' );
dbms_output.put_line(chrMb_proposal||' '||chrMb_sort_code||' '||chrMb_funding_code||' '||chrMb_tranx_date||' tc '||chrMb_source_tranx_no);
					numAct_cost:=1/0;
				END IF;
				numAct_hrs:=numAct_hrs*numPercentage/100;
-- add amount to either labor or material amounts. On 9/26/2014: also add hours
				numSaved_hrs:=numSaved_hrs+numAct_hrs;
...
-- Per Meenal on 12/03/2009 set bill_amount to numMb_amount (just like amount).
			UPDATE ae_s_fnd_a
				SET post_flag='Y',bill_date=datEnd_date,
					amount=numMb_amount,fc_amount=numMb_amount,bill_amount=numMb_amount,
					funding_code=chrMb_funding_code,
					offsetfunding_code=chrMb_offsetfunding_code,
					edit_date=SYSDATE,edit_clerk='PROC_SUB100'
...
-- Copy rows of Plantsub to output file
	OPEN curPlantsub(intTenant_arg);
	LOOP
		FETCH curPlantsub INTO chrTr_card_code,chrTr_short_code,
			...
			chrTr_item_desc,chrTr_vendor_item_id,
...
-- Append contents for file buffer
		...
		chrFile_buffer:=CONCAT(chrFile_buffer,chrTr_blanks_1);
...
PROCEDURE proc_SaveCurrentInfo(chrProposal_arg VARCHAR2,chrSort_code_arg VARCHAR2,chrFunding_code_arg VARCHAR2,chrOffsetfunding_code_arg VARCHAR2, chrSourceTranx_no_arg number,
	chrSubledger_type_arg VARCHAR2) IS
BEGIN
	...
	chrSaved_subledger_type:=chrSubledger_type_arg;
	numSaved_hrs:=0;
...
PROCEDURE proc_WriteOutCurrentRecord(intTenant_arg NUMBER) IS
BEGIN
...
--			--AWI - AIM 6.0 - MULTITENANCY - END
	...
	IF(chrSaved_subledger_type='L') THEN
		IF(numSaved_hrs=0) THEN
			numSaved_hrs:=1;
		END IF;
		numTr_unit_price:=ROUND(numChg_acct_labr/numSaved_hrs,4);
--		numTr_qty_vouchered:=ROUND(numSaved_hrs,4);
		numTr_qty_vouchered:=ROUND(numChg_acct_labr/numTr_unit_price,4);
		chrTr_invoice_id:='LABOR;'||chrSaved_sort_code;
		numChg_acct_labr:=ROUND(numTr_unit_price * numTr_qty_vouchered,2);
	ELSE
		numTr_unit_price:=numChg_acct_matr;
		numTr_qty_vouchered:=1;
		chrTr_invoice_id:='MATERIALS;'||chrSaved_sort_code;
	END IF;
...
--		WHERE trans_no=chrMB_Source_tranx_no;
	...
	--2500 add-to-totals
	proc_AddToTotals(intTenant_arg);
...
-- Right adjust previous numeric variable when putting into an alphabetic field (for file insertion).
	...
	chrTr_unit_price:=func_RightAdjust(numTr_unit_price,17,4);
	chrTr_qty_vouchered:=func_RightAdjust(numTr_qty_vouchered,16,4);
...
-- Parse Chart field to retrieve fund code, org_code, subclass, program code, project grant and account
	...
	--5100
	proc_BreakUpChartField(intTenant_arg);
...
chrFile_buffer:=chrFile_buffer||chrTr_item_desc||chrTr_vendor_item_id||chrTr_manufacturer_name;
...
PROCEDURE proc_WriteOutZeroRecs(intTenant_arg NUMBER) IS
BEGIN
...
-- Parse Chart field to retrieve fund code, org_code, subclass, program code, project grant and account
PROCEDURE proc_BreakUpChartField(intTenant_arg NUMBER) IS
...
BEGIN
	...
	chrBuffer:=REPLACE(chrBuffer,'  .',' 0.');
	RETURN chrBuffer;
END func_RightAdjust;

PROCEDURE proc_AddAmounts(intTenant_arg NUMBER) IS
BEGIN
...
PROCEDURE proc_AddToTotals(intTenant_arg NUMBER) IS
BEGIN
	BEGIN
...
-- but this should never happen
		...
		dbms_output.put_line(intTenant_arg);
	END;
	SELECT shop INTO chrAe_shop
		FROM ae_p_phs_e
		WHERE proposal=chrSaved_proposal
...
PROCEDURE proc_TestChartField(chrArg VARCHAR2,intTenant_arg NUMBER) IS
begin
...
DBMS_OUTPUT.PUT_LINE('MULTITENANT_ID: '||intTenant_arg);
...
--PROCEDURE proc_UpldSubITSErrors IS
--filSub_errors	UTL_FILE.FILE_TYPE;
--chrBuffer	VARCHAR2(500);
--chrCar_code	VARCHAR2(1);
--SHORT_CODE                                         VARCHAR2(6)
--ACCOUNT                                            VARCHAR2(6)
...
--SUBLEDGER_SORT_CODE                                VARCHAR2(30)
--PROPOSAL                                           VARCHAR2(25)
--BEGIN
--	EXECUTE IMMEDIATE 'TRUNCATE TABLE um_sub_its_errors';
--	filInvoiceOut:=utl_file.fopen('PROC_DIR','','R');
--	LOOP
--		UTL_FILE.GET_LINE(filSub_errors,chrBuffer);
--		INSERT INTO um_its_sub_errors
--			VALUES()
--	END LOOP;
--	UTL_FILE.FCLOSE(filSub_errors);
--	COMMIT;
--	UPDATE ae_s_fnd_a
--		SET bill_date='1-jan-2048', bill_Amount=0,post_flag='N',edit_Date=sysdate,edit_Clerk='AVIGGERS'
--		WHERE (trunc(bill_Date)=trunc(sysdate))
--		AND (proposal,sort_code)
--			IN(SELECT proposal,SUBSTR(sort_code,INSTR(sort_code,';')+1)
--				FROM um_its_sub_errors
--				WHERE vendor_id in(select vendor_id from pap_bill_vendor_id where offsetfunding_Code=ae_S_Fnd_a.offsetfunding_Code)
--		);
--
--	fmax_3.proc_monthlybillupdate('YYYYMMDD','YYYYMMDD',1);
--END proc_UpldSubITSErrors;
END fmaxC_18;