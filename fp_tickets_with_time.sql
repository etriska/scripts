...
TITLE,
  REPLACE(REPLACE(REPLACE(Private__bComments, '&#58;',':'),'&#34;','"'), '&#39;', '''') PRIVATE_COMMENTS,
  CASE
  ...
  sum(c.mrTIMESPENT) TIMESPENT
FROM MASTER374 a
JOIN MASTER374_ABDATA b ON a.MRID = b.MRID
join Footprints.MASTER374_TIMETRACKING c on a.MRID = c.MRID
WHERE a.MRSTATUS not in ('Closed','_DELETED_','Complete', 'Waiting__bon__bVendor','_SOLVED_','_REQUEST_')
and a.Primary__bPerson in ('etriska','monalh','meenalsm','aviggers','jdaunora','msebolt')
group by a.mrID, a.MRPRIORITY, a.mrSTATUS, a.PLANTOPS__BDEPARTMENT, b.full__Bname, a.Primary__bPerson, a.MRSUBMITDATE, DATE(a.Due__bDate)
ORDER BY a.Primary__bPerson, a.mrPRIORITY, a.mrSUBMITDATE