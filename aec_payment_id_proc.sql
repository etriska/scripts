...
--set dates, since user didn't specify date values or they were invalid
	...
	proc_SaveCurrentInfo(chrMb_proposal,chrMb_sort_code,chrMb_funding_code,chrMb_offsetfunding_code);
...
PROCEDURE proc_SaveCurrentInfo(chrProposal_arg VARCHAR2,chrSort_code_arg VARCHAR2,chrFunding_code_arg VARCHAR2,chrOffsetfunding_code_arg VARCHAR2) IS
BEGIN
...
-- and set others like invoice date (todays date) and default unit price.
	...
	BEGIN
	SELECT CUSTOM007 INTO CHRAE_CUSTOM007
		FROM AE_I_EXT_E_UDF
	...
	END;
	-- End of 5010
	IF(chrAe_custom007 IS NULL) THEN
		chrAe_custom007:='                              ';
	END IF;

	chrTr_dept_ref_no:=chrAe_custom007;
